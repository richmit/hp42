...
(defun MJR-generate-42-menu-code (top-lab tbl leaf-function exit-function gen-code)
   ;; leaf-function "stay" or "exit", exit-function "exit" or "up"
  (let* ((no-local 't)
         (allow-lleaf 't)
         (min-free-lab  2)
         (m-code "")
    ...
    (cl-labels ((add-m-code (a) (setq m-code (concat m-code a "\n")))
                ...
                (gen-mnu (plbl lbl menu) (let* ((num-menu-keys (1- (length menu)))
                                           ...
                                           (cl-loop for mkey-elt in (cdr menu)
                                                    ...
                                                    for is-lleaf = (and allow-lleaf (vectorp mkey-elt) (numberp (cl-second (aref mkey-elt 1))))
                                                    when (= page-key 0)
...
#+BEGIN_SRC elisp :var tbl=menu :colnames y :results output verbatum :wrap "src hp42s :tangle SOMEFILE"
(MJR-generate-42-menu-code "PROG_NAME" tbl "stay" "up" (lambda (x) ""))
#+END_SRC

#+RESULTS:
#+begin_src hp42s :tangle SOMEFILE
@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@ (ref:PROG_NAME)
