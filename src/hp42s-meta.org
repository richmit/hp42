...
(defun MJR-generate-42-menu-code (top-lab tbl leaf-function exit-function allow-local-labs gen-code)
" leaf-function:    String:  stay, exit
  exit-function:    String:  exit, up
  allow-local-labs: Boolean: Do we recognize numbers for local lable targets"
   (let* ((no-local 't)
         (min-free-lab  2)
         (m-code "")
         (x-code ""))
    (cl-labels ((add-m-code (a) (setq m-code (concat m-code a "\n")))
                ...
                (gen-mnu (plbl lbl menu) (let* ((num-menu-keys (1- (length menu)))
                                           ...
                                           (cl-loop for mkey-elt in (cdr menu)
                                                    ...
                                                    for is-lleaf = (and allow-local-labs (vectorp mkey-elt) (numberp (cl-second (aref mkey-elt 1))))
                                                    when (= page-key 0)
                                                    ...
                                                    do (progn (if (string-match-p "^[0-9][0-9]$" mkey-str)
                                                                  (progn (add-m-code (message "XEQ %s" mkey-str))
                                                                         (setq no-local nil))
...
(MJR-generate-42-menu-code "PROG_NAME" tbl "stay" "up" 't (lambda (x) ""))
#+END_SRC
