...
|------+--------+--------------------------------------------+--------+--------------------|
| IDEN | MATIDN | Create an nxn identity matrix              | X: N   | X: identity matrix |
| DIAG | MATDIG | Create diagonal matrix with given elements | X: VEC | X: diagonal matrix |
| TR   | MATTR  | Compute the trace of a matrix              | X: M   | X: trace           |
| CPLY | MCPOLY | Compute matrix Characteristic polynomial   | X: M   | X: polynomial      |
|      |        |                                            |        |                    |
| EDIT | LBL 96 |                                            |        |                    |
|------+--------+--------------------------------------------+--------+--------------------|
...
*** =MCPOLY=: Characteristic polynomial

=MCPOLY= uses the Faddeev–LeVerrier algorithm to compute the characteristic polynomial of a matrix.  The polynomial is a matrix of coefficients suitable for use
