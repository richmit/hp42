...
| TAN2  |  97 |                                     |
| HYPOT |  98 |                                     |
| HAV   |  90 | Haversine = 1/2-cos(X)/2            |
| AHAV  |  89 | Inverse Haversine = 2*asin(sqrt(X)) |
| ▒▒▒▒  |     |                                     |
| ▒▒▒▒  |     |                                     |

*** Menu Code
#+BEGIN_SRC elisp :var tbl=TRIG :colnames y :results output verbatum :wrap "src hp42s :tangle ../src_42s/math/math.hp42s"
(MJR-generate-42-menu-code "TRIG" tbl "stay" "exit" 't #'MJR-custom-x-gen)
#+END_SRC
...
| Menu | Lab | Description                                          |
|------+-----+------------------------------------------------------|
| lnYX |  99 | Base Y Logarithm of X                                |
| ▒▒▒▒ |     |                                                      |
| ln2  |  98 | Base 2 logarithm                                     |
| LN   |     | Base 3 logarithm                                     |
...
| ▒▒▒▒ |     |                                                      |
| 2↑X  |  97 | Raise 2 to the power of X                            |
| EXP  |     | Raise e to the power of X                            |
| 10↑X |     | Raise 10 to the power of X                           |
| ▒▒▒▒ |     |                                                      |
|------+-----+------------------------------------------------------|
| Y√X  |  96 | Yth roots prefering real & pure imaginary answers    |
| ▒▒▒▒ |     |                                                      |
| 2√X  |  95 | Square root prefering real & pure imaginary answers  |
| 3√X  |  94 | Cube root prefering real & pure imaginary answers    |
| 5√X  |  93 | Fifth root prefering real & pure imaginary answers   |
| 7√X  |  92 | Seventh root prefering real & pure imaginary answers |

*** Menu Code
#+BEGIN_SRC elisp :var tbl=LPRT :colnames y :results output verbatum :wrap "src hp42s :tangle ../src_42s/math/math.hp42s"
(MJR-generate-42-menu-code "LPRT" tbl "stay" "exit" 't #'MJR-custom-x-gen)
#+END_SRC
...
Y↑X
RTN
LBL 07               @@@@ Action for menu key EXP
XEQ "EXP"
RTN
LBL 08               @@@@ Action for menu key 10↑X
10↑X
RTN
...
|------+-----+--------------------------------------------+---------------+--------------------+------------|
| IDEN |  99 | Create an nxn identity matrix              | X: N          | X: identity matrix | *COMPLETE* |
| DIAG |  98 | Create diagonal matrix with given elements | X: VEC        | X: diagonal matrix | *COMPLETE* |
| ROWB |  87 | Bind two matrixes top to bottom            | Y: MAT X: MAT | X: adjoined matrix | *TODO*     |
| COLB |  86 | Bind two matrixes top to bottom            | Y: MAT X: MAT | X: adjoined matrix | *TODO*     |

*** Code for Menu

#+BEGIN_SRC elisp :var tbl=NLA :colnames y :results output verbatum :wrap "src hp42s :tangle ../src_42s/math/math.hp42s"
(MJR-generate-42-menu-code "NLA" tbl "stay" "exit" 't #'MJR-custom-x-gen)
#+END_SRC
...
|-------+---------+------------------------------+------------------+---------------------|
| QUADE |      99 | Solve quadratic: Ax^2+Bx+C=0 | Z: A Y: B X: C   | Y: root 1 X: root 2 |
| EVAL  |      98 | Evaluate polynomial P @ X    | Y: P matrix X: X | X: P(x)             |

*** Code for Menu

#+BEGIN_SRC elisp :var tbl=POLY :colnames y :results output verbatum :wrap "src hp42s :tangle ../src_42s/math/math.hp42s"
(MJR-generate-42-menu-code "POLY" tbl "stay" "exit" 't #'MJR-custom-x-gen)
#+END_SRC
...
| Menu  | Target |                                                                     |
|-------+--------+---------------------------------------------------------------------|
| →V    |     99 | Put stack elements X, Y, & Z into a vector: [Z, Y, X]               |
| V→    |     98 | Vector contents to stack. [A, B, C] => X: C, Y: B, Z: A             |
| DOT   |        | Dot product                                                         |
| CROSS |        | Cross product                                                       |
| MAG   |   FNRM | Euculidian magnitude                                                |
| VVIEW |     96 | View a vector one element at a time -- press =R/S= for next element |
|-------+--------+---------------------------------------------------------------------|

** Code for Menu

#+BEGIN_SRC elisp :var tbl=VEC3 :colnames y :results output verbatum :wrap "src hp42s :tangle ../src_42s/math/math.hp42s"
(MJR-generate-42-menu-code "VEC3" tbl "stay" "exit" 't #'MJR-custom-x-gen)
#+END_SRC
