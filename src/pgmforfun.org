...
Sometimes you just need to quickly evaluate a function for several diffrent input values.  

** =FEVL=: Quicly Evaluate A Function

I frequently use =VARMENU= to repeatidly evluate a formula, with diffrent input values.  Functionally that means typeing in the value, and then hitting the
variable button twice.  That's not super efficient when repeatidly changeing the same variable.  That's wehre this little function comes in.  
...
Features:
   ...
   - Control over axis and grid line drawin
   - Quick access to solver and integrater from plot
   - Factor zoom
   - Default window
   - Minimal & efficient interface
   - Integrated with other tools on this page -- solver & integrater
