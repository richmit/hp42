...
Notes:
  ...
  - Most functions don't drop the stack.  They just push results onto the stack.  
  - Some functions, like =[PX>DC]=, may take one or two arguments depending upon what is on the stack when the function is executed.
...
Scenario: We have a Washington-Franklin vertical coil, and whish to verify that the flat edges are parallel.  We have a 2400 DPI scan.

Our first step is to identify two points in each edge.  The points should be seporated as far apart as possible.  Suppose this results in the
following:
...
Note that we didn't try to line the points up horizontally -- our uper left poit is at 160 while our upper right point is at 122!
