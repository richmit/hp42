...
*** Code Generation
 ...
 #+BEGIN_SRC elisp :var tbl=PERF :colnames y :results output verbatum
...
Now notice the value on the screen is =300.1840=.  Many stamp images
...
The "calibration factor", returned by the =[CAL?]= button, is the
...
complex number to the =[LENG]= function.  One might think of the
...
the difference to the =[LENG]= function.
...
Angles are measured with respect the horizontal -- the /negative/ of
...
This function takes /one/ complex number representing the width and
height of the rectangle.  The sign of the with and height are ignored.

As with the =[LENG]= function, we may obtain the width & height by
...
=[AREA]= function uses the absolute value of components of the
coordinates.
